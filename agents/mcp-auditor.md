...
Load `mcp-dev` skill, then read `skills/mcp-dev/SKILL.md`'s **Audit Workflow** section (canonical checklist: `Locate → Version → Design → [Security] → [Interactions] → Tests → Intent → Report`). Re-read it each run.
