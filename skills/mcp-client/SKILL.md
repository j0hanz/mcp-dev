...
2. **Initialize Client**: Initialize `new Client(...)` with your configurations, setting target credentials or capacities parameters (declaring elicitation capacity upfront (roots is deprecated per SEP-2577 — pass paths via tool arguments/resource URIs/host config instead)).
3. **Establish Transport Connection**: Instantiate transport layers (`StreamableHTTPClientTransport`, `StdioClientTransport` from `@modelcontextprotocol/client/stdio` subpath, `SSEClientTransport (fallback for SSE-only servers — try StreamableHTTP first, retry with SSE)`) and connect with `await client.connect(transport)`.
4. **Register Hook Interceptors**: Register handlers afterward via `setRequestHandler('elicitation/create', …)` (sampling/createMessage and roots/list are deprecated per SEP-2577; register only if you need legacy support) to negotiate auto-fulfilment.
...
- [ ] Handshakes utilize the correct negotiation mode (`'auto'`, `'legacy'`, or `{ pin: '2026-07-28' }` for modern-only). Avoid `'auto'` negotiation on spawn-per-invocation CLI tools to prevent legacy stalls.
- [ ] Client declares capacities (elicitation mandatory; sampling and roots deprecated per SEP-2577 — declare only if used) in the constructor option BEFORE registering handler endpoints.
