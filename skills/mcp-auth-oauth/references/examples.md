...
```ts
import { createMcpExpressApp, getOAuthProtectedResourceMetadataUrl,
         mcpAuthMetadataRouter, requireBearerAuth } from '@modelcontextprotocol/express';
import type { AuthInfo } from '@modelcontextprotocol/server';

const mcpServerUrl = new URL('https://api.example.com/mcp');

async function verifyAccessToken(token: string): Promise<AuthInfo> {   // the one function to supply
...
new PrivateKeyJwtProvider({ clientId, privateKey /* PEM | Uint8Array | JWK */, algorithm: 'RS256',
                            jwtLifetimeSeconds: 300, claims: {} });

// bring-your-own token — minimal AuthProvider:
const authProvider = { token: async () => getStoredToken(), onUnauthorized: async ctx => refresh() };
```
