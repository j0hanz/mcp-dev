...
```ts
import { acceptedContent, inputRequired, type CallToolResult, type InputRequiredResult } from '@modelcontextprotocol/server';
...
const server = new McpServer({ name: 'releases', version: '1.0.0' },
  { requestState: { verify: stateCodec.verify } });   // runs before every stateful entry
