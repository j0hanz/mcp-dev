...
mcp.server.setRequestHandler('acme/search', { params: SearchParams, result: SearchResult },
...
client.setNotificationHandler('acme/searchProgress', { params: z.object({ stage: z.string(), pct: z.number() }) },
...
class SocketTransport implements Transport {
  ...
  async start() {
    this.socket.on('data', chunk => {
      ...
      let m; while ((m = this.readBuffer.readMessage()) !== null) this.onmessage?.(m);
    });
